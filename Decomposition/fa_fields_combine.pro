...
if keyword_set(spline) then interp = 2l
    ...
    flbdir = getenv('FASTLIB')

    if keyword_set(flbdir) then flbdir = flbdir + '/'  $
    else begin
        message, "Environment variable FASTLIB not set--can't find libsybidl.so"
        return
    endelse

    status = call_external(flbdir + lmdl, 'ff_time_align', $
		dat1.time, $			; ARG 0
		result, $			; ARG 1
		long(npts1), $			; ARG 2
